...
left_join(all_NY1850, NY1850_match_counts, 
          by = c("borrower_section" = "match_section")) %>% 
  select(section = borrower_section, n) %>% 
  mutate(n = ifelse(n < 6, NA, n)) %>% 
  mutate(n = cut(n, c(seq(0, 40, 5), 85))) %>% 
  mutate(.,
  ...
  scale_fill_brewer(palette = "YlOrBr", na.value = "white", 
                    labels = c("6-10", "11-15", "16-20", "21-25", "26-30",
                               "31-35", "36-40", "40+")) +
  labs(x = NULL, y = NULL, fill = "Number of times borrowed") + 
  coord_equal() + 
  theme_minimal() + 
  ...
  ggtitle("Most frequently borrowed sections in NY1850")
