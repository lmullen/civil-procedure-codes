---
title: "Compare pairwise with Jaccard bag similarity"
...
We are going to load the corpus from the cache, and create a comparison matrix with Jaccard bag similarity. (We will do other comparisons in different notebook so we can do this in parallel.) We will cache the matrix for use in later analyses.

We are going to test this with both shingled and skip n-grams. Here are skip n-grams.

```{r}
cache_skip <- "cache/comparison-matrix-skip-jaccard-bag.rds"
if (!file.exists(cache_skip)) {
  corpus_skip <- readRDS("cache/corpus-skip-ngrams-n7k3.rds")
  m_skip <- pairwise_compare(corpus_skip, jaccard_bag_similarity)
...
```{r}
cache_shingles <- "cache/comparison-matrix-shingles-jaccard-bag.rds"
