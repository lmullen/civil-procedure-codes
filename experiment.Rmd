...
```{r}
ny_to_nc_matches <- intersect(codes_grams$NC1868.txt, codes_grams$NY1850c.txt) 
ny_to_nc_possible <- unique(c(codes_grams$NC1868.txt, codes_grams$NY1850c.txt))
...
```{r}
ny_to_mi_matches <- intersect(codes_grams$MI1853.txt, codes_grams$NY1850c.txt) 
ny_to_mi_possible <- unique(c(codes_grams$MI1853.txt, codes_grams$NY1850c.txt))
...
```{r}
cf_ny_nc <- compare_codes_by_shared_ngrams(codes_grams$NY1850c.txt,
...
```{r}
cf_ny_ca <- compare_codes_by_shared_ngrams(codes_grams$NY1850c.txt,
...
```{r, fig.height = 3, fig.width = 8}
density_plot_color <- rgb(0.5, 0.5, 0.5, 0.1)
ca_matches <- codes_grams$NC1868.txt %in% cf_ny_nc$matches  
ca_matches %>% 
  plot(type = "h",
       col = density_plot_color,
       ann = FALSE,
...
```{r, fig.height = 3, fig.width = 8}
cf_ny_mi <- compare_codes_by_shared_ngrams(codes_grams$NY1850c.txt,
